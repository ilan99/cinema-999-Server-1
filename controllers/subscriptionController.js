...
router.route("/ref").get(async (req, res) => {
  const { data: subscriptions } =
    await subscriptionServices.getAllSubscriptionsByRef();
  return res.json(subscriptions);
});
...
router.route("/:id").get(async (req, res) => {
  ...
  const { data: subscription } = await subscriptionServices.getSubscriptionById(
    id
  );
...
router.route("/").post(async (req, res) => {
  ...
  try {
    const { data } = await subscriptionServices.addSubscription(
      newSubscription
    );
